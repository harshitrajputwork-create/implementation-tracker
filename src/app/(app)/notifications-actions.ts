...
export async function getNotificationsDataAction(): Promise<{
  ...
  const [{ data: notifRows }, { count }, { data: noteDeadlines }, { data: taskDeadlines }] = await Promise.all([
    ...
    supabase.from('client_notes').select('id, content, deadline, client_id, trial_account_id, clients(name), trial_accounts(name)').eq('author_id', user.id).eq('deadline_done', false).not('deadline', 'is', null).lte('deadline', horizonStr).order('deadline'),
  ...
  const fromNotes: UpcomingReminder[] = (noteDeadlines ?? []).map((n: any) => ({
    id: `note-${n.id}`,
    label: n.content,
    clientName: n.clients?.name ?? n.trial_accounts?.name ?? null,
    deadline: n.deadline,
    linkPath: n.trial_account_id ? `/trial/${n.trial_account_id}#note-${n.id}` : `/clients/${n.client_id}#note-${n.id}`,
